...
train1 <- as.data.frame(unclass(train))
...
summary(model1)
model2 <- lm(formula = log_saleprice ~ SalePrice + GarageFinish + Neighborhood + 
    BsmtQual + Heating + ExterQual + BedroomAbvGr + YearRemodAdd + 
    GarageCond + CentralAir + Functional + OverallQual + BldgType + 
    MSZoning + GrLivArea + ScreenPorch + OverallCond + 
    YearBuilt + LowQualFinSF + GarageType + Fireplaces + SaleCondition + 
    MasVnrArea + MasVnrType + LotConfig + Fence + 
    BsmtFullBath + X2ndFlrSF + PavedDrive + RoofStyle + 
    SaleType + FullBath + KitchenAbvGr + MSSubClass, data = train1)
summary(model2)
...
kaggle_test$log_saleprice <- 0
kaggle_test$SalePrice <- 0
model_formula <- model2
predictor_matrix <- model.matrix(model_formula, data=train1)[, -1]
predictor_matrix2 <- model.matrix(model_formula, data=kaggle_test)[, -1]
lambda_inputs <- 10^seq(-2, 10, length = 100)

LASSO_fit <- glmnet(x=predictor_matrix, y=train1$log_saleprice, alpha = 1, lambda = lambda_inputs)
...
y_hat <- predict(LASSO_fit, newx=predictor_matrix, s=lambda_star_1SE) %>% 
  as.vector()
kaggle_test$SalePrice <- exp(y_hat[1:1459])
hist(y_hat)
hist(test$log_saleprice, xlab = "y-actual" , main="Historgram of test log(SalesPrice)")
RMSE <- sqrt(mean((y_hat - test$log_saleprice)^2))

RMSE2 <-  sqrt(mean((y_hat - test$log_saleprice)^2))
...
```{r}
library(readr)
sub <- kaggle_test %>% select(Id, SalePrice)
sub <-as.matrix(sub)
...
for(i in 1:length(lambda_inputs)){
  for(j in 1:5){
    ...
    train_set <- train1 %>% 
      filter(fold == j)
    predict <- predictor_matrix %>% 
  
    validation_set <- train1 %>% 
      filter(fold !=j)
    
  LASSO_fit <- glmnet(x=predict, y=train_set$log_saleprice, alpha = 1, lambda = lambda_inputs[i])
