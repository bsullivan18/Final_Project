...
library(factoextra)
pca <- train %>% 
  select(log_saleprice,GrLivArea,GarageArea,LotFrontage, OverallQual,OverallCond,BsmtFinSF1,BsmtFinSF2,TotalBsmtSF,TotRmsAbvGrd,PoolArea,YearBuilt, LotArea)
...
train %>% 
  count(fold)
predictor_matrix <- model.matrix(model_formula, data=train)[, -1] 
predictor_matrix <- as.data.frame(predictor_matrix) %>% 
  sample_frac(1) %>% 
  mutate(fold = rep(1:10, length=n()))
predictor_matrix %>% 
  count(fold)
...
for(i in 1:length(vector)){
  for(j in 1:10){
    ...
    validation_set <- train %>% 
      filter(fold !=j)
    pred <- as.data.frame(predictor_matrix) %>% 
      filter(fold == j)

    mod <-  get_LASSO_coefficients(glmnet(x=pred, y=train_set$log_saleprice, alpha=1, lambda=vector))
    # Get predictions
   
    
  }   
}
 predictions <- predict(mod, newx=predictor_matrix, s=vector[i])
```
